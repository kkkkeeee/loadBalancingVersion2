...
c     should we inject particles at this time step?
      ...
      if (stage.eq.1) then
         ...
         p_time = p_time + dnekclock() - p_time_dum

         ! Update where particle is stored at
         ptdum(3) = dnekclock()
            call move_particles_inproc
         pttime(3) = pttime(3) + dnekclock() - ptdum(3)
         p_comm_time = p_comm_time + dnekclock() - ptdum(3)
         ...
         if (two_way.gt.1) then
            ...
            p_comm_time = p_comm_time + dnekclock() - ptdum(5)
...
c     write(6,*) "#particles: ", nid, nelt, n
