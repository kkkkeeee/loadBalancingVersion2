...
c     Solve the Euler equations
      ...
      !fluid_time = dnekclock()
      nstage = 3
      do stage=1,nstage
c        fluid_time_dum = dnekclock_sync()
         if (stage.eq.1) call copy(res3(1,1,1,1,1),U(1,1,1,1,1),n1)

         rhst_dum = dnekclock()
         call compute_rhs_and_dt
         rhst = rhst + dnekclock() - rhst_dum
c        fluid_time = fluid_time + dnekclock() - fluid_time_dum
c        if(istep .eq. 4) 
c    $ write(6, *) "compute_rhs ", stage, nid, fluid_time 
c    $  - fluid_time_dum
...
#ifdef CMTPART
c        p_time_dum = dnekclock_sync()
         call usr_particles_solver
c        p_time = p_time + dnekclock() - p_time_dum
c        if(istep .eq. 4) 
c    $    write(6, *) "usr_particles ", stage, nid, p_time
#endif
...
c-----------------------------------------------------------------------
            ...
            enddo
         enddo
         fluid_time = fluid_time + dnekclock() - fluid_time_dum
c     if(nid .eq. 8 .or. nid.eq. 0)
c    $ write(6,*) "mtime_loop ", istep, stage, nid 
c    $ ,dnekclock() - fluid_time_dum, fluid_time
      enddo
      ...
      fluid_time = fluid_time + dnekclock() - fluid_time_dum
c     if(nid .eq. 8 .or. nid.eq. 0)
c    $ write(6,*) "mtime_copy ", istep, nid 
c    $ ,dnekclock() - fluid_time_dum, fluid_time
      if(n .ne. 0 .and. mod(istep, 100).eq. 0) then
      write(6,*) "comtime: ", istep, nid, fluid_time
     $ ,nelt, n
     $ , p_time, fluid_time/nelt, p_time/n
      endif
      tot_comp_time = fluid_time + p_time
      f_comm_time = 0.0
      p_comm_time = 0.0
...
!        primitive vars = rho, u, v, w, p, T, phi_g
      ...
      fluid_time = fluid_time + dnekclock() - fluid_time_dum
c     if(nid .eq. 8 .or. nid.eq. 0)
c    $ write(6,*) "mtime_part 1", istep, stage, nid 
c    $ ,dnekclock() - fluid_time_dum, fluid_time
...
!-----------------------------------------------------------------------
      ...
      f_comm_time = f_comm_time +dnekclock() - f_comm_time_dum
...
C> \f$\mathbf{U}^+\f$; store in CMTSURFLX
      fluid_time = fluid_time + dnekclock() - fluid_time_dum
c     if(nid .eq. 8 .or. nid.eq. 0)
c    $ write(6,*) "mtime_part 2", istep, stage, nid 
c    $ ,dnekclock() - fluid_time_dum, fluid_time
...
C> res1+=\f$\int_{\Gamma} \{\{\mathbf{A}\nabla \mathbf{U}\}\} \cdot \left[v\right] dA\f$
      fluid_time = fluid_time + dnekclock() - fluid_time_dum
c     if(nid .eq. 8 .or. nid.eq. 0)
c    $ write(6,*) "mtime_part 3", istep, stage, nid 
c    $ ,dnekclock() - fluid_time_dum, fluid_time

      call igu_cmt(flux(iwp),graduf,flux(iwm)) !may involve
                                               !communication
...
!     call dumpresidue(dumchars,999)
      fluid_time = fluid_time + dnekclock() - fluid_time_dum
c     if(nid .eq. 8 .or. nid.eq. 0)
c    $ write(6,*) "mtime_part 4", istep, stage, nid 
c    $ ,dnekclock() - fluid_time_dum, fluid_time
