...
c-----------------------------------------------------------------------
      ...
      real comp_time_max, comp_time_avg, imbal_pct
c     real starttime
...
#ifdef CMTPART
#ifdef CMTNEK
      !get the maximum and average computational time
      comp_time_max = glmax(tot_comp_time, 1)
      comp_time_avg = glsum(tot_comp_time, 1)/np
c     write(6,*) "nid ", nid, " tot_comp_time ", tot_comp_time
c    $,np, comp_time_max, comp_time_avg  
c     write(6,*) "time1:", istep, nid, fluid_time, nelt, p_time, n
      if(np .gt.1 .and. comp_time_max .ne. 0.0) then
         imbal_pct = (comp_time_max - comp_time_avg)/comp_time_max*
     $      np*1.0/(np-1)
      else
         imbal_pct = 0.0
      endif
      if(nid.eq.0) then
         write(6,*) "imbal_pct ", imbal_pct, comp_time_max,
     $      comp_time_avg
      endif
c     if(imbal_pct .gt. 0.5 ) then
c        resetFindpts = 1
c        call reinitialize
c     endif
#endif
